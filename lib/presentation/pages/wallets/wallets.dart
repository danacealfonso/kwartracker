...
import 'package:kwartracker/presentation/pages/transactions/transaction_add_details.dart';
import 'package:kwartracker/presentation/pages/transactions/transactions.dart';
import 'package:kwartracker/presentation/pages/wallets/wallet_save.dart';
...
import 'package:kwartracker/presentation/widgets/custom_progress_bar.dart';
    ...
    Widget content() {
      return Consumer<FirestoreData>(builder:
        ...
        final List<Widget> imageSliders =
            firestoreData.walletsList.map((dynamic item) {
          showGoal.add(item['type'].toString().toLowerCase() == 'goal');
          targetAmount.add(item['targetAmount'] != null
              ? '${item['currencySign']} ${NumberFormat.currency(customPattern: '#,###.##').format(item['targetAmount'])}'
              : '0.00');
          return Container(
            child: Container(
                width: 240,
              ...
              Stack(children: <Widget>[
                      ...
                      onPageChanged:
                        ...
                        if (mounted) {
                          setState(() {
                            _current = index;
                            overAllBalance =
                                firestoreData.overallBalance[index];
                          });
                        }
                      }),
                ),
                Padding(
                  ...
                  child: Row(
                    ...
                    children: firestoreData.walletsList.map((dynamic item) {
                ...
                child: Row(children: <Widget>[
                            ...
                            Navigator.push(
                                ...
                                MyRoute<dynamic>(
                                  builder: (BuildContext context) =>
                                      WalletSavePage(
                                          walletID: firestoreData
                                              .walletIDs[_current]),
                                  routeSettings:
                                      const RouteSettings(name: '/walletSave'),
                                ));
                          },
                          child: Image.asset('images/icons/ic_edit.png',
                              width: 10, height: 10, fit: BoxFit.fill))),
                  const Expanded(
                  ...
                  Container(
                      ...
                      child: FloatingActionButton(
                          ...
                          child: Image.asset('images/icons/ic_add_grey.png',
                              width: 10, height: 10, fit: BoxFit.fill)),
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(50),
                        boxShadow: const <BoxShadow>[
                  ...
                  const Expanded(
                      child: Text(
                    'Add Transaction',
                    style: TextStyle(color: ColorConstants.grey6, fontSize: 12),
                  ))
                ]),
              ),
              if (showGoal.length - 1 >= _current)
                if (showGoal[_current] == true)
                  Container(
